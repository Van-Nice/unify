...
const savedSitesKey = "unify.savedSites";
...
// Saved sites tree item class
class SavedSiteItem extends vscode.TreeItem {
  constructor(readonly item: SavedItem) {
    super(
      item.name,
      isFolder(item)
...
// Saved sites tree data provider
class SavedSitesProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  getChildren(
    ...
    if (!element) {
      return savedSites.map((item) => new SavedSiteItem(item));
    }
...
// Saved sites drag and drop controller
class SavedSitesDragAndDropController
  implements vscode.TreeDragAndDropController<vscode.TreeItem>
{
  readonly dropMimeTypes = ["application/vnd.unify.saved-item"];
  readonly dragMimeTypes = ["application/vnd.unify.saved-item"];
  private readonly mimeType = "application/vnd.unify.saved-item";
...
// Helper functions for saved sites
...
function isDescendant(parent: Folder, targetId: string): boolean {
  ...
  for (const child of parent.children) {
    if (isFolder(child) && isDescendant(child, targetId)) {
      return true;
    }
  }
  return false;
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Initialize saved sites
  savedSites = context.globalState.get(savedSitesKey, []);
  savedSitesProvider = new SavedSitesProvider();

  // Create saved sites tree view
  const savedSitesTreeView = vscode.window.createTreeView("unify.savedSites", {
  ...
  context.subscriptions.push(savedSitesTreeView);

  // Register saved sites commands
  context.subscriptions.push(
    vscode.commands.registerCommand("unify.openSite", (site: Site) => {
      void launch(context, site.url);
    })
  );
      ...
      provideHover(document, position) {
        ...
        for (const diagnostic of documentDiagnostics) {
          if (
            ...
            telemetryReporter.sendTelemetryEvent("user/webhint/hover", {
              hint: (diagnostic.code as DiagnosticCodeType).value,
            });
          }
        }
        return null;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      () => {
        telemetryReporter.sendTelemetryEvent("user/buttonPress", {
          "VSCode.buttonCode":
            LaunchConfigManager.instance.getLaunchConfig() === "None"
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration(`${SETTINGS_STORE_NAME}.webhint`)) {
      if (
        vscode.workspace.getConfiguration(SETTINGS_STORE_NAME).get("webhint")
      ) {
        void startWebhint(context);
      } else {
        void stopWebhint();
...
export async function launchHtml(fileUri: vscode.Uri): Promise<void> {
  ...
  if (!vscode.env.remoteName) {
    ...
    void vscode.debug
      ...
      .then(() => vscode.debug.startDebugging(undefined, devToolsAttachConfig));
  } else {
    // Parse the filename from the remoteName, file authority and path e.g. file://wsl.localhost/ubuntu-20.04/test/index.html
    const url = `file://${vscode.env.remoteName}.localhost/${
      fileUri.authority.split("+")[1]
          ...
          case "vscode-webhint/ignore-browsers-project": {
            if (args.length > 1) {
              ...
              telemetryReporter.sendTelemetryEvent(
                ...
                { hint: hintName, value: browserList.join(",") }
              );
            }
            break;
          }
        ...
        .then((button) => {
          if (button === "Disable Extension") {
            void vscode.workspace
              .getConfiguration(SETTINGS_STORE_NAME)
              .update("webhint", false, vscode.ConfigurationTarget.Global);
          }
